function [effort, distance, velocity, CoT, isFalling, stepLength, stepFrequency] = analyse(sln, parameters, to_plot)
...
%         sln.Y{i}(j,:)
    ...
    stepLength = mean(step_length);
    stepFrequency = mean(step_frequency);
end
...
velocity = mean(dxh); %dxh_mean(end);
CoT = effort/distance;
if (min(zh) <= 0)
    isFalling = true;
else
...
if to_plot    
    ...
    disp("mean step length : " + stepLength);
    disp("mean step frequency : " + stepFrequency);
