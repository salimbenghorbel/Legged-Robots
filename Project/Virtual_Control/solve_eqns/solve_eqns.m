...
function sln = solve_eqns(q0, dq0, num_steps, parameters, disturbance_list)
...
t10 = inf;
for i = 1:num_steps
    [T, Y, TE, YE] = ode45(@(t, y) eqns(t, y, t0, t10, y0, i, parameters, disturbance_list, h), t0 + tspan, y0, options);
    if i == 10
        t10 = T(1);
    end
    sln.T{i} = T;
